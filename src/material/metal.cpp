...
std::shared_ptr<BSDF> Metal::getBSDF(const Intersection & isec, TransportMode mode) const
{
	auto bsdf = std::make_shared<BSDF>(isec);
	Float rough = roughness->evaluate(isec).r;
	Float alpha = rough = GGX::roughnessToAlpha(rough);
	auto fresnel = std::make_shared<FresnelConductor>(1.0f, eta->evaluate(isec), k->evaluate(isec));
	auto distrib = std::make_shared<GGX>(alpha);
	bsdf->addBxDF(std::make_shared<MicrofacetReflection>(1.0f, distrib, fresnel));
	return bsdf;
}
...
std::shared_ptr<Metal> createMetalMaterial(const std::shared_ptr<Texture>& eta, const std::shared_ptr<Texture>& k, const std::shared_ptr<FloatTexture>& roughness, const ParamSet &param)
{
	auto r = roughness;
	if (roughness == nullptr)
		r = std::make_shared<FloatTexture>(0.1f);
	if (eta != nullptr && k != nullptr && roughness != nullptr)
		return std::make_shared<Metal>(eta, k, r);

	Spectrum N, K;
	if (param.hasParam("metalName")) {
		std::string metal = param.getParam("metalName");
		if (metal == "copper") {
			static Spectrum copperN = Spectrum::fromSampled(SopraWavelengths, CopperN, SopraSamples);
			static Spectrum copperK = Spectrum::fromSampled(SopraWavelengths, CopperK, SopraSamples);
			N = copperN;
			K = copperK;
		}
		else if (metal == "amorphousCarbon") {
			static Spectrum amorphousCarbonN = Spectrum::fromSampled(AmorphousCarbonWavelengths, AmorphousCarbonN, AmorphousCarbonSamples);
			static Spectrum amorphousCarbonK = Spectrum::fromSampled(AmorphousCarbonWavelengths, AmorphousCarbonK, AmorphousCarbonSamples);
			N = amorphousCarbonN;
			K = amorphousCarbonK;
		}
		else if (metal == "silver") {
			static Spectrum silverN = Spectrum::fromSampled(SopraWavelengths, SilverN, SopraSamples);
			static Spectrum silverK = Spectrum::fromSampled(SopraWavelengths, SilverK, SopraSamples);
			N = silverN;
			K = silverK;
		}
		else if (metal == "gold") {
			static Spectrum goldN = Spectrum::fromSampled(SopraWavelengths, GoldN, SopraSamples);
			static Spectrum goldK = Spectrum::fromSampled(SopraWavelengths, GoldK, SopraSamples);
			N = goldN;
			K = goldK;
		}
		else if (metal == "aluminium") {
			static Spectrum aluminiumN = Spectrum::fromSampled(SopraWavelengths, AluminiumN, SopraSamples);
			static Spectrum aluminiumK = Spectrum::fromSampled(SopraWavelengths, AluminiumK, SopraSamples);
			N = aluminiumN;
			K = aluminiumK;
		}
		else
			CHECK_INFO(N.isBlack(), "lack of metal material!!");
	}
	auto ETA = std::make_shared<ConstantTexture>(N);
	auto KT = std::make_shared<ConstantTexture>(K);
	return std::make_shared<Metal>(ETA, KT, r);
}
ORION_NAMESPACE_END
